...
// established.
func ParseWinEvents(events []attest.Event) (*WinEvents, error) {
	var (
		out = WinEvents{
			LoadedModules: map[string]WinModuleLoad{},
			ELAM:          map[string]WinELAM{},
	...
	for _, e := range events {
		...
		// END -- digestVerify := e.digestEquals(e.Data)
		switch e.Index {
		case 12: // 'early boot' events
			...
			case EventTag:
				...
				if err != nil {
					...
					return nil, fmt.Errorf("invalid tagged event structure at event: %w", err)
				}
				//if digestVerify != nil {
				//	return nil, fmt.Errorf("invalid digest for tagged event: %w", err)
				//}
				if !digestVerify {
					return nil, fmt.Errorf("invalid digest for tagged event: %w", err)
				}
				if err := out.readWinEventBlock(s, e.Index); err != nil {
					return nil, fmt.Errorf("invalid SIPA events in event: %w", err)
				}
			case Separator:
				if seenSeparator.PCR12 {
					return nil, fmt.Errorf("duplicate WBCL separator at event")
				}
				seenSeparator.PCR12 = true
				if !bytes.Equal(e.Data, []byte("WBCL")) {
					return nil, fmt.Errorf("invalid WBCL separator data at event: %v", e.Data)
				}
				//if digestVerify != nil {
				//	return nil, fmt.Errorf("invalid separator digest at event: %v", digestVerify)
				//}
				if !digestVerify {
					return nil, fmt.Errorf("invalid separator digest at event: %v", digestVerify)
				}
